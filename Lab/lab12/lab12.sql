...
CREATE TABLE big AS
  SELECT finals.course FROM finals, sizes WHERE finals.hall = sizes.room GROUP BY finals.course HAVING sum(seats) > 1000;

CREATE TABLE remaining AS
  SELECT course, sum(sizes.seats) - max(sizes.seats) AS remaining
  FROM finals, sizes WHERE finals.hall = sizes.room GROUP BY finals.course;

...
def balanced(m):
    ...
    torque_balanced = (length(left_arm) * left_mass) == (length(right_arm) * right_mass)
    
    substructure_balanced = balanced(end(left_arm)) and balanced(end(right_arm))
    
    return torque_balanced and substructure_balanced
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    max_path_total = max(max_path_sum(b) for b in branches(t))
    return label(t) + max_path_total
